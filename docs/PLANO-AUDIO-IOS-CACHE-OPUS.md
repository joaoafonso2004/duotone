...
- **Entrega 3 — feita, falta o aparelho** (a pedido do dono, antes da prova da 2). Conversor WebM→fMP4 puro (`lib/webmOpus.ts`, `lib/opusMp4.ts`), escolha do 251 só em iOS 17+ e qualidade alta, cache `yt-opus-v1-` ao lado do AAC, renovação sempre no mesmo formato, e recuo automático para AAC: na conversão, e quando o motor recusa o ficheiro (duas recusas seguidas → 14 dias só AAC). O Opus não toca enquanto descarrega e não entra no crossfade antes de uma faixa Opus ter tocado. Testes: `test-webm-opus.ts` (contra o ffmpeg), `test-escolha-codec.ts`, casos Opus em `test-transmitir-audio.mjs`. Ficou por fazer, e só se justifica com provas: CAF em produção, e o formato do tap do EQ (o plano pedia registar o ASBD — não se mexeu no Swift sem o poder compilar).
