...
import { imageCrop, LARGURA_DA_CAPA, LARGURA_DO_AVATAR, RACIO_DA_CAPA, RACIO_DO_AVATAR } from './profileImageCrop';
...
export async function prepareProfileImage(image:SelectedProfileImage,kind:ProfileMediaKind,y=0.5,x=0.5,zoom=1):Promise<ArrayBuffer> {
  const img=await loadImage(image.uri);
  const crop=imageCrop(image.width,image.height,kind==='avatar'?RACIO_DO_AVATAR:RACIO_DA_CAPA,x,y,zoom);
  const canvas=document.createElement('canvas');canvas.width=kind==='avatar'?LARGURA_DO_AVATAR:LARGURA_DA_CAPA;canvas.height=kind==='avatar'?LARGURA_DO_AVATAR:Math.round(LARGURA_DA_CAPA/RACIO_DA_CAPA);
