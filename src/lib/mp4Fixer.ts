...
function findAndFixBoxes(
  ...
  while (offset + 8 <= end) {
    ...
    if (type === 'moov' || type === 'trak' || type === 'mdia' || type === 'mvex') {
      ...
      findAndFixBoxes(buffer, boxContentStart, boxContentEnd, targetDurationSec);
    } else if (type === 'mvhd') {
      // Movie Header
      const version = buffer[boxContentStart];
      if (version === 1) {
        const timescale = read32(buffer, boxContentStart + 20);
        const newDuration = Math.round(targetDurationSec * timescale);
      ...
      // Track Header (usa timescale global, habitualmente 1000)
      const version = buffer[boxContentStart];
      const newDuration = Math.round(targetDurationSec * 1000);
      if (version === 1) {
...
export function fixMp4Duration(buffer: Uint8Array, durationSeconds: number): void {
  ...
  try {
    findAndFixBoxes(buffer, 0, buffer.length, durationSeconds);
  } catch (err) {
