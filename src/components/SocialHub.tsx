...
export function SocialHub({onProfile,onPlaylist,onArtist,visible=true,initialFriend,initialGroup}:{onProfile:(id:string)=>void;onPlaylist:(id:string)=>void;onArtist:(name:string)=>void;visible?:boolean;initialFriend?:string;initialGroup?:string}) {
  ...
  const run=async(action:()=>Promise<unknown>)=>{if(busy)return;setBusy(true);setError('');try{await action();await social.refresh();}catch(e:any){setError(e.message || 'Não foi possível concluir.');}finally{setBusy(false);}};
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      ...
      catch(e:any){if(active)setError(e.message || 'Não foi possível atualizar a conversa.');}
  ...
  const title=friend?.name || group?.name || 'Conversa';
  return <View style={s.body}>
    <View style={[s.row,{paddingHorizontal:24,paddingBottom:8}]}><SocialButton onPress={()=>setTab('friends')}>Conversas</SocialButton><SocialButton quiet onPress={()=>setTab('add')}>Adicionar amigos</SocialButton><Pressable accessibilityLabel="Atualizar Social" onPress={()=>void social.refresh()}><Ionicons name="refresh" size={20} color="#B6A7D0"/></Pressable></View>
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={s.content}>
      {(error||social.error)&&<Text accessibilityRole="alert" style={s.error}>{error||social.error}</Text>}
      {social.loading&&<ActivityIndicator color="#B7A4E3"/>}
      {tab==='add'?<>
        <Text style={s.title}>Encontra a tua companhia musical</Text><Text style={s.muted}>Pesquisa pelo nome ou username.</Text>
        <TextInput accessibilityLabel="Pesquisar amigos" value={query} onChangeText={setQuery} style={s.input} placeholder="Nome ou username" placeholderTextColor="#8E879C" autoCapitalize="none"/>
        {results.map(p=><View key={p.id} style={[s.card,s.row]}><Pressable onPress={()=>onProfile(p.id)}><FriendAvatar avatarUrl={p.avatar_url} name={p.name} size={44}/></Pressable><View style={{flex:1}}><Text style={s.text}>{p.name}</Text><Text style={s.muted}>@{p.username}</Text></View><SocialButton disabled={busy||social.friends.some(f=>f.friendId===p.id)} onPress={()=>void run(()=>sendFriendRequest(p.id))}>{social.friends.some(f=>f.friendId===p.id)?'Adicionado':'Adicionar'}</SocialButton></View>)}
      </>:<>
        {pending.length>0&&<Text style={s.label}>Pedidos de amizade</Text>}
        {pending.map(f=><View key={f.friendId} style={s.card}><View style={s.row}><FriendAvatar avatarUrl={f.avatarUrl} name={f.name} size={40}/><View style={{flex:1}}><Text style={s.text}>{f.name}</Text><Text style={s.muted}>{f.isSender?'Pedido enviado':'Quer ser teu amigo'}</Text></View></View><View style={s.row}>{!f.isSender&&<SocialButton disabled={busy} onPress={()=>void run(()=>acceptFriendRequest(f.friendId))}>Aceitar</SocialButton>}<SocialButton quiet disabled={busy} onPress={()=>void run(()=>declineOrRemoveFriendship(f.friendId))}>{f.isSender?'Cancelar pedido':'Recusar'}</SocialButton></View></View>)}
        <View style={[s.row,{justifyContent:'space-between'}]}><Text style={s.label}>Grupos</Text><SocialButton quiet onPress={()=>{setGroupEditor('new');setMembers([]);setGroupName('');}}>+ Novo grupo</SocialButton></View>
        {social.groups.map(g=><Pressable key={g.id} style={[s.card,s.row]} onPress={()=>open('group',g.id)}><Ionicons name="people" size={28} color="#B3A0D2"/><View style={{flex:1}}><Text style={s.text}>{g.name}</Text><Text numberOfLines={1} style={s.muted}>{g.membros.map(m=>m.name).join(', ')}</Text></View>{!!unread.get(`group:${g.id}`)&&<Text style={s.badge}>{unread.get(`group:${g.id}`)}</Text>}</Pressable>)}
        <Text style={s.label}>Amigos · {accepted.length}</Text>
        {!accepted.length&&!social.loading&&<View style={s.card}><Text style={s.title}>Música sabe melhor acompanhada</Text><Text style={s.muted}>Adiciona um amigo para partilhar músicas e começar uma conversa.</Text><SocialButton onPress={()=>setTab('add')}>Adicionar amigo</SocialButton></View>}
        {accepted.map(f=><View key={f.friendId} style={[s.card,s.row]}>
          <Pressable accessibilityLabel={`Ver perfil de ${f.name}`} onPress={()=>onProfile(f.friendId)}><FriendAvatar avatarUrl={f.avatarUrl} name={f.name} size={46}/></Pressable>
          <Pressable style={{flex:1,gap:3}} onPress={()=>open('friend',f.friendId)}><View style={s.row}><Text style={[s.text,{fontWeight:'700',flex:1}]}>{f.name}</Text>{!!unread.get(f.friendId)&&<Text style={s.badge}>{unread.get(f.friendId)}</Text>}</View><Text numberOfLines={2} style={[s.muted,f.online&&{color:'#88D9B8'}]}>{f.online?'● Online agora':ultimaAtividade(f.lastSeenAt,social.now)}</Text>{f.currentlyPlaying&&<Text numberOfLines={1} style={s.muted}>♫ {f.currentlyPlaying.title}</Text>}</Pressable>
          <Pressable accessibilityLabel={`Remover ${f.name} dos amigos`} hitSlop={12} onPress={()=>setConfirm({id:f.friendId,group:false})}><Ionicons name="person-remove-outline" size={18} color="#9D91B0"/></Pressable>
        </View>)}
        {social.contacts.filter(p=>!accepted.some(f=>f.friendId===p.id)).map(p=><Pressable key={p.id} onPress={()=>open('friend',p.id)} style={[s.card,s.row]}><FriendAvatar avatarUrl={p.avatar_url} name={p.name} size={42}/><View style={{flex:1}}><Text style={s.text}>{p.name}</Text><Text style={s.muted}>Conversa anterior</Text></View>{!!unread.get(p.id)&&<Text style={s.badge}>{unread.get(p.id)}</Text>}</Pressable>)}
      </>}
    </ScrollView>
    <SocialModal visible={!!conversation&&visible&&!track&&!groupEditor&&!confirm} title={title} onClose={()=>useSocial.setState({conversation:null})}>
      <View style={{height:480,flexShrink:1,padding:16,gap:12}}>
        {friend&&<Pressable accessibilityLabel={`Ver perfil de ${friend.name}`} style={s.row} onPress={()=>onProfile(friend.friendId)}><FriendAvatar avatarUrl={friend.avatarUrl} name={friend.name} size={40}/><View style={{flex:1}}><Text style={s.text}>{friend.name} · Ver perfil</Text><Text style={s.muted}>{friend.online?'● Online agora':ultimaAtividade(friend.lastSeenAt,social.now)}</Text>{friend.currentlyPlaying&&<Text numberOfLines={1} style={s.muted}>♫ {friend.currentlyPlaying.title}</Text>}</View></Pressable>}
        {group&&<View style={s.row}><SocialButton quiet onPress={()=>{setMembers([]);setGroupEditor(group.id);}}>Adicionar pessoas</SocialButton><SocialButton quiet onPress={()=>setConfirm({id:group.id,group:true})}>Sair do grupo</SocialButton></View>}
        {!!error&&<Text style={s.error}>{error}</Text>}{chatLoading&&<ActivityIndicator color="#B7A4E3"/>}
        <FlatList inverted ListFooterComponent={hasOlder?<SocialButton disabled={older} onPress={()=>void loadOlder()}>{older?'A carregar…':'Mensagens anteriores'}</SocialButton>:null} data={[...messages].reverse()} keyExtractor={m=>m.id} contentContainerStyle={{gap:12,paddingVertical:10}} style={{flex:1}} keyboardShouldPersistTaps="handled" renderItem={({item:m})=><View style={{alignSelf:m.sender.id===myId?'flex-end':'flex-start',maxWidth:'92%',backgroundColor:m.sender.id===myId?'#322844':'#22202A',padding:12,borderRadius:15,gap:8}}>
          ...
          {m.playlistId&&<SocialButton quiet onPress={()=>onPlaylist(m.playlistId!)}>Abrir playlist</SocialButton>}
          <Text style={[s.muted,{fontSize:10}]}>{new Date(m.createdAt).toLocaleTimeString('pt-PT',{hour:'2-digit',minute:'2-digit'})}</Text>
        </View>}/>
        <View style={s.row}><TextInput accessibilityLabel="Mensagem" placeholder="Escreve uma mensagem…" placeholderTextColor="#90869F" value={draft} onChangeText={setDraft} multiline maxLength={4000} style={[s.input,{flex:1,maxHeight:90}]} editable={!busy}/><SocialButton disabled={busy||!draft.trim()} onPress={()=>void send()}>Enviar</SocialButton></View>
      </View>
    </SocialModal>
    <SocialModal visible={!!confirm} title={confirm?.group?'Sair do grupo?':'Remover amizade?'} onClose={()=>setConfirm(null)}><View style={{padding:20,gap:12}}><Text style={s.muted}>As mensagens anteriores continuam guardadas.</Text><SocialButton disabled={busy} onPress={()=>void run(async()=>{if(!confirm)return;if(confirm.group)await sairDoGrupo(confirm.id);else await declineOrRemoveFriendship(confirm.id);setConfirm(null);useSocial.setState({conversation:null});})}>Confirmar</SocialButton><SocialButton quiet onPress={()=>setConfirm(null)}>Cancelar</SocialButton></View></SocialModal>
    <SocialModal visible={!!groupEditor} title={groupEditor==='new'?'Novo grupo':'Adicionar pessoas'} onClose={()=>setGroupEditor(null)}><ScrollView contentContainerStyle={{padding:20,gap:12}}>{groupEditor==='new'&&<TextInput accessibilityLabel="Nome do grupo" value={groupName} onChangeText={setGroupName} placeholder="Nome do grupo" placeholderTextColor="#90869F" style={s.input}/>}{accepted.filter(f=>groupEditor==='new'||!social.groups.find(g=>g.id===groupEditor)?.membros.some(m=>m.id===f.friendId)).map(f=><SocialButton key={f.friendId} onPress={()=>setMembers(m=>m.includes(f.friendId)?m.filter(id=>id!==f.friendId):[...m,f.friendId])}>{members.includes(f.friendId)?'✓ ':''}{f.name}</SocialButton>)}<SocialButton disabled={busy||!members.length||(groupEditor==='new'&&!groupName.trim())} onPress={()=>void run(async()=>{if(groupEditor==='new')await criarGrupo(groupName,members);else if(groupEditor)await acrescentarAoGrupo(groupEditor,members);setGroupEditor(null);})}>Guardar</SocialButton>{!!error&&<Text style={s.error}>{error}</Text>}</ScrollView></SocialModal>
