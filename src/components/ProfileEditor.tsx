...
export function ProfileEditor({profile,onClose,onSaved}:{profile:SocialProfile;onClose:()=>void;onSaved:()=>void}) {
  ...
  return <SocialModal visible title="Edit profile" onClose={()=>{if(!stage)onClose();}}>
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{padding:20,gap:20}}>
      ...
      <View style={[s.row,{flexWrap:'wrap'}]}><SocialButton disabled={!!stage} onPress={()=>void select('cover')}>Carregar capa</SocialButton><SocialButton quiet disabled={!!stage} onPress={()=>{setCover(null);setValue({...value,cover_path:null});}}>Remove cover</SocialButton></View>
      ...
      <Text style={s.label}>Sobre ti</Text><TextInput accessibilityLabel="Bio" editable={!stage} value={value.bio} onChangeText={bio=>setValue({...value,bio})} maxLength={180} multiline placeholder="A line about you or your music." placeholderTextColor={colors.textSecondary} style={[s.input,{minHeight:80}]}/>
