...
export function ProfileEditor({profile,onClose,onSaved}:{profile:SocialProfile;onClose:()=>void;onSaved:()=>void}) {
  ...
  const save=async()=>{
    ...
    let saving=false;
    try {
      setError('');setStage('A preparar imagens…');
      const next={...value};
      for(const kind of ['avatar','cover'] as const){
        ...
        setStage(kind==='avatar'?'A enviar fotografia…':'A enviar capa…');
      ...
      setError(e.message || 'Não foi possível guardar. Podes tentar novamente.');
    } finally {setStage('');}
  };
  return <SocialModal visible title="Editar perfil" onClose={()=>{if(!stage)onClose();}}>
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{padding:20,gap:20}}>
      <View style={{aspectRatio:8/3,borderRadius:18,overflow:'hidden',backgroundColor:value.accent+'33'}}>
        ...
        <View style={[s.row,{position:'absolute',bottom:12,left:12,right:12,backgroundColor:'#121117CC',borderRadius:14,padding:10}]}>
          ...
          <View style={{flex:1}}><Text style={[s.title,{fontSize:21}]}>{name || 'O teu nome'}</Text><Text style={s.muted}>@{profile.profile.username}</Text></View>
        </View>
      </View>
      <Text style={s.label}>Fotografia</Text>
      <View style={[s.row,{flexWrap:'wrap'}]}><SocialButton disabled={!!stage} onPress={()=>void select('avatar')}>Carregar fotografia</SocialButton><SocialButton quiet disabled={!!stage} onPress={()=>{setAvatar(null);setValue({...value,avatar_path:null,legacy_avatar_url:null});}}>Usar emoji</SocialButton></View>
      {avatar && <View style={[s.row,{flexWrap:'wrap'}]}><Text style={s.muted}>Ajustar recorte</Text><SocialButton disabled={!!stage} onPress={()=>setAvatarX(Math.max(0,avatarX-0.1))}>←</SocialButton><SocialButton disabled={!!stage} onPress={()=>setAvatarX(Math.min(1,avatarX+0.1))}>→</SocialButton><SocialButton disabled={!!stage} onPress={()=>setAvatarY(Math.max(0,avatarY-0.1))}>↑</SocialButton><SocialButton disabled={!!stage} onPress={()=>setAvatarY(Math.min(1,avatarY+0.1))}>↓</SocialButton></View>}
      {!avatar && !value.avatar_path && !value.legacy_avatar_url && <><View style={[s.row,{flexWrap:'wrap'}]}>{AVATAR_EMOJIS.map(emoji=><SocialButton disabled={!!stage} key={emoji} onPress={()=>setValue({...value,emoji})}>{emoji}</SocialButton>)}</View><View style={[s.row,{flexWrap:'wrap'}]}>{AVATAR_GRADIENTS.map((_,i)=><Pressable disabled={!!stage} accessibilityRole="button" accessibilityLabel={`Gradiente ${i+1}`} accessibilityState={{selected:value.gradient_index===i}} key={i} onPress={()=>setValue({...value,gradient_index:i})} style={{padding:3,borderWidth:2,borderRadius:26,borderColor:value.gradient_index===i?'#F3EFFF':'transparent'}}><FriendAvatar avatarUrl={`emoji:${value.emoji}:${i}`} name={name} size={34}/></Pressable>)}</View></>}
      <Text style={s.label}>Capa do perfil</Text>
      <View style={[s.row,{flexWrap:'wrap'}]}><SocialButton disabled={!!stage} onPress={()=>void select('cover')}>Carregar capa</SocialButton><SocialButton quiet disabled={!!stage} onPress={()=>{setCover(null);setValue({...value,cover_path:null});}}>Remover capa</SocialButton></View>
      {cover && <View style={s.row}><SocialButton disabled={!!stage} onPress={()=>setValue({...value,cover_position:Math.max(0,value.cover_position-0.1)})}>↑ Subir</SocialButton><SocialButton disabled={!!stage} onPress={()=>setValue({...value,cover_position:Math.min(1,value.cover_position+0.1)})}>↓ Descer</SocialButton><Text style={s.muted}>{Math.round(value.cover_position*100)}%</Text></View>}
      <Text style={s.label}>Nome</Text><TextInput accessibilityLabel="Nome do perfil" editable={!stage} value={name} onChangeText={setName} maxLength={40} style={s.input}/>
      <Text style={s.label}>Username</Text><TextInput accessibilityLabel="Nome de utilizador" autoComplete="off" textContentType="none" editable={!stage} autoCapitalize="none" autoCorrect={false} value={username} onChangeText={setUsername} maxLength={30} style={s.input}/>
      <Text style={s.label}>Sobre ti</Text><TextInput accessibilityLabel="Biografia" editable={!stage} value={value.bio} onChangeText={bio=>setValue({...value,bio})} maxLength={180} multiline placeholder="Uma frase sobre ti ou sobre a tua música." placeholderTextColor="#8E879C" style={[s.input,{minHeight:80}]}/>
      <Text style={s.label}>Cor de destaque</Text><View style={[s.row,{flexWrap:'wrap'}]}>{['#A78BFA','#F472B6','#60A5FA','#34D399','#FBBF24'].map(accent=><SocialButton key={accent} onPress={()=>setValue({...value,accent})}><Text style={{color:accent}}>● {value.accent===accent?'✓':''}</Text></SocialButton>)}</View>
      {!!error && <Text accessibilityRole="alert" style={s.error}>{error}</Text>}
    </ScrollView>
    <View style={[s.row,{padding:16,borderTopWidth:1,borderColor:'#292633',justifyContent:'flex-end'}]}><SocialButton quiet disabled={!!stage} onPress={onClose}>Cancelar</SocialButton><SocialButton disabled={!!stage||name.trim().length<2} onPress={()=>void save()}>{stage || 'Guardar alterações'}</SocialButton></View>
  </SocialModal>;
}
