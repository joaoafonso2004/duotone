...
import { linhaDeMeta } from '../../components/CabecalhoDaPlaylist';
...
export function PlaylistPage({ id, title, back, share, navigate, ...props }: { id: string; title: string; back: () => void; share: (target: ShareTarget) => void; navigate: (route: Route) => void } & CommonPageProps) {
  ...
  /**
   * A duracao total, e so quando se sabem TODAS as faixas: uma soma feita com
   * metade delas sem duracao seria um numero errado com ar de exacto. A mesma
   * conta e a mesma frase do telemovel -- ver `linhaDeMeta`.
   */
   ...
   * playlist é de outra pessoa -- nas minhas não há botão.
   */
  const [copias,setCopias]=useState<Set<string>|null>(null);
  const [aGuardar,setAGuardar]=useState(false);
  useEffect(()=>{
    if(!ownerId||!userId||ownerId===userId){setCopias(null);return;}
    let vivo=true;
  ...
  const guardar=async()=>{
    ...
    try{
      ...
      // your copy" sabe para onde ir, sem guardar o id em lado nenhum.
      ...
      if(jaTinha)navigate({name:'playlist',id:copia,title:`${playlistTitle} (Shared)`});
      else props.notify('Saved to your playlists.');
    }catch(e:any){props.notify(mensagemDeFalhaAoGuardar(e));}
  ...
  const filteredTracks = useMemo(() => {
    ...
    // Pelo que se VÊ (título limpo, artista a sério), como as Liked Songs.
    return ordenarFaixas(filtradas, sortMode, { titulo: (t) => tituloDaFaixa(t), artista: (t) => displayArtist(t) });
  }, [tracks, query, sortMode]);
  // As colunas da tabela ordenam o mesmo estado que o botão. O botão continua,
  // porque tem duas ordens que não são colunas: a da playlist e "Recently added".
  const colunaAtiva: ColunaOrdenavel | null =
    sortMode === 'title' || sortMode === 'artist' || sortMode === 'duration' ? sortMode : null;
