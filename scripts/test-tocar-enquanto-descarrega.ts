...
import {
  ...
  depoisDeTransmitir, descreverSaude, lerSaude, podeTransmitir, primeiraNota,
...
// A linha do relatório.
assert.equal(descreverSaude(SAUDE_INICIAL, agora, false), 'off (this build has no streaming module)');
assert.equal(descreverSaude(SAUDE_INICIAL, agora, true), 'on');
assert.match(descreverSaude({ falhasSeguidas: 1, desligadoAte: 0 }, agora, true), /^on \(1 recent/);
assert.match(descreverSaude(guardada, agora, true), /^off after repeated player failures, back on 20\d\d-/);
